...
Our goal now is to take two bots and compose them together in parallel
as if they are a single bot with both behaviors. At the type level, what
this looks like is combining the three type parameters of our `Bots`
with a binary associative type constructor. For example, we could use
...
[smash library](https://hackage.haskell.org/package/smash). We can also
mix and match your binary type constructors however we desire.
...
In our particular case what we want is a way to conditional run *either*
of the two bots based on the input we receive. This indicates that we
want to use `Either` for `i` and `o`. However, we don\'t want to use
`Either` for our state `s`. Instead we should use `(,)` to ensure that
regardless of which bot we choose to execute, we have it\'s required
state available.
...
By equiping a `Functor`, or in our case a form of `Trifunctor`, with a
binary associative type constructor(s) we are constructing what is
called a `Monoidal Functor`. We can leverage the `Semigroupal` typeclass
from the
...
library to implement `(\/)` and the other related combinators I just
described.

We have 3 type constructors we wish to monoidally combine (`s`, `i`, and
`o`) so we choose the `Data.Trifunctor.Monoidal.Semigroupal` class and
then we can write instances parameterized with whatever binary
